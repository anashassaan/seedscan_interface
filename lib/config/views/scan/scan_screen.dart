...
class _ScanScreenState extends State<ScanScreen>
  ...
  // Local camera controller â€” created fresh each time this screen opens
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            icon: Icon(Icons.cameraswitch_rounded, color: cs.onSurface),
          ),
      ...
      body: Stack(
        children: [
          ...
          const Positioned.fill(child: _OverlayCutout(cornerRadius: 20)),
                    ...
                    Positioned.fill(
                      child: Container(
                        decoration: BoxDecoration(
                          borderRadius: BorderRadius.circular(20),
                          border: Border.all(
                              color: cs.onSurface.withOpacity(0.18), width: 2),
                        ),
                      ),
                    ),
                  ],
  ...
  /// Show QR scan result â€” detects My Garden and Community plant QR codes.
  void _showQRResult(BuildContext context, String qrData, ScanController scan) {
    // â”€â”€ Community plant QR: SEEDSCAN|communityId|id|plantName|plantType|bestSeason|SEED/PLANT|plantAge|timestamp
...
class _ScanMyGardenQRDialogState extends State<_ScanMyGardenQRDialog> {
  ...
  Future<void> _plantNow() async {
    ...
    try {
      ...
      // 2. Get GPS location â€” force enable if location service is off
      double lat = 0.0;
      double lng = 0.0;
      try {
        ...
        if (!_isChecking && _existsInDb && !_planted)
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text(_isPlanting ? 'Planting...' : 'ðŸŒ± Plant Now?',
...
/// Community Plant QR Dialog â€” handles admin-generated SEEDSCAN| QR codes.
...
class _ScanCommunityQRDialogState extends State<_ScanCommunityQRDialog> {
  ...
  Future<void> _plantAndJoin() async {
    ...
    try {
      ...
      // 5. Create activity log â€” use the plant's Appwrite $id so admin history works
      try {
